...
func SendVerificationEmail(email string, token string) error {
	...
	link := fmt.Sprintf("https://oauth-go-backend.vercel.app/verify-email?token=%s", token)
...
func SendResetPasswordEmail(email string, token string) error {
	...
	link := fmt.Sprintf("https://oauth-go-backend.vercel.app/reset-password?token=%s", token)
